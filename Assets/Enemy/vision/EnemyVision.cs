...
namespace Enemy.vision
{
    ...
    public class EnemyVision : MonoBehaviour
    {
        ...
        public bool group_detect; //Group detect will make an enemy follow another enemy chasing even if didnt see player
        ...
        private common.scripts.Enemy enemy;

        private VisionTarget seen_character;
        private VisionCone vision;

        private float detect_timer;
        private float vision_timer;
        private float wait_timer;

        private static readonly List<EnemyVision> enemy_list = new List<EnemyVision>();

        private void Awake()
        {
            enemy_list.Add(this);
            enemy = GetComponent<common.scripts.Enemy>();
            if(enemy != null)
        ...
        private void Update()
        {
            ...
            if(enemy == null || !GameManager.getInstance().canEnemiesMove)
                return;

            wait_timer -= Time.deltaTime;
            
            
            //While patrolling, detect targets
            ...
            {
                var can_see_target = CanSeeVisionTarget(seen_character);
                ...
                if (seen_character != null){
                    var distance = GetSeenDistance(seen_character.gameObject);
                    Debug.Log(distance);
                    onSeeTarget?.Invoke(seen_character, distance);
                }
        ...
        private void DetectVisionTargetOnly()
        {
            ...
            var target = CanSeeAnyVisionTarget();
            if (target == null || !(detect_timer > detect_time)) return;
            var dist = GetSeenDistance(target.gameObject);
        ...
        private void DetectVisionTarget(){
            ...
            //Detect character
            foreach (var character in VisionTarget.GetAll().Where(character => character != seen_character).Where(character => CanSeeVisionTarget(character))){
                Alert(character);
                var distance = GetSeenDistance(character.gameObject);
        ...
        private void DetectTouchTarget(){
            ...
            foreach (var character in VisionTarget.GetAll().Where(character => CanTouchObject(character.gameObject)).Where(character => onTouchTarget != null)){
        ...
        private void DetectOtherEnemies()
        {
            ...
            foreach (var singleEnemy in common.scripts.Enemy.GetAll())
            {
                if (singleEnemy != enemy && singleEnemy.GetState() == EnemyState.Chase)
                {
                    if (singleEnemy.follow_target != null && CanSeeObject(singleEnemy.gameObject, vision_range, vision_angle))
                    {
                        VisionTarget target = singleEnemy.follow_target.GetComponent<VisionTarget>();
                        Chase(target);
        ...
        public bool CanSeeObject(GameObject obj, float see_range, float see_angle)
        {
            ...
            float losheight = position.y - GetEye().y;
            ...
            {
                bool raycast = Physics.Raycast(new Ray(GetEye(), dir.normalized), out var hit, dir.magnitude, vision_mask.value);
                if (!raycast)
                    return true; //No obstacles in the way (in case character not in layer)
                if (hit.collider.gameObject == obj || hit.collider.transform.IsChildOf(obj.transform)) //See character
        ...
        {
            var is_near = CanSeeObject(target, vision_near_range, vision_angle);
            var is_touch = CanTouchObject(target);
            var distance = is_touch ? 0 : (is_near ? 1 : 2);
            return distance;
        }

        //Call this function from another script to manually alert of the target presense
        private void Alert(VisionTarget target)
        {
            if (target != null)
            {
        ...
        private void Alert(Vector3 target)
        {
            if(enemy != null)
                enemy.Alert(target);
            vision_timer = 0f;
        ...
        public common.scripts.Enemy GetEnemy()
        {
            return enemy;
        }
